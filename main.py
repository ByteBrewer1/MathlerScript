...
def getstate():
    ...
    for i in blocks:
        attribute = driver.find_element(
            by=By.XPATH, value=i).get_attribute("class")
        if "bg-green" in attribute:
            ...
            for i in range(6):
                driver.find_element(
                    by=By.XPATH, value="/html/body").send_keys(Keys.BACKSPACE)
            return None
    row += 1
    return state
...
def main():
    ...
    actions.move_to_element(driver.find_element(
        by=By.XPATH, value="/html/body/div[2]/div/div/div/div[2]/div[2]/div/div")).move_by_offset(200, 0).click().perform()
    problem = int(driver.find_element(
        by=By.XPATH, value="/html/body/div/div/div[2]/h2").get_attribute("innerHTML").split()[-1])
    while True:
        ...
        driver.find_element(
            by=By.XPATH, value="/html/body").send_keys(Keys.ENTER)
        try:
            c += sasta_ai(getstate())
